...
| A-P1-2 | Avoid duplicating all Pi semantics into Protobuf; raw Pi events should remain available. | **Resolved/refined.** Protobuf rejected. Exact UTF-8 is `rawJson`+digest/reference; parsed projection is explicitly separate and non-authoritative. |
...
| B-P0-4 | Pi has no built-in approval gate; borrowed “Approve” UI would be false security. | **Superseded by final audit.** Host gates direct actions; pinned Pi final-hook patch + global preload broker runs after all handlers in nested sessions. Separate approval protocol; fail closed. See FA-P0-1. |
