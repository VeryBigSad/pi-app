...
A cold app process and return after more than five minutes backgrounded require a fresh assertion. Authorization is attached to the current mTLS connection and expires after five minutes of background lock. Device-authenticated connections may exchange hello, auth, revocation, and generic wake metadata; they cannot fetch session content or mutate Pi before user authentication.
...
1. Mac CLI creates one five-minute, single-use invitation, invalidating any previous pending invitation. QR carries verified App Link fragment, invitation ID/nonce, signed relay bootstrap, Mac instance ID, and provisional server-certificate fingerprint; fragments never reach Pages.
...
Certificates include URI SAN identities (`urn:pimobile:mac:{instanceId}` and `urn:pimobile:device:{deviceId}`). Both peers validate chain, TLS 1.3, EKU, validity, expected SAN identity, and revocation after every handshake. Revocation closes active connections and blocks future handshakes. Renewal occurs while the old certificate is valid and user-authenticated; otherwise re-pair.
...
The Mac maintains one P-256 challenge-authenticated control WSS to the relay, with heartbeat, bounded liveness timeout, jittered exponential backoff, and cold reconnect. A paired Android data request is challenge-authenticated with its registered route key. The relay sends an opaque one-use connection notice over control; Mac opens an outbound data WSS bound to that notice; relay pairs the two sockets in memory and byte-splices. First-device bootstrap uses the signed pairing invitation because no device route key exists yet.

Relay durable state is limited to route registration key IDs/public keys and revocation. It stores no shared bearer/HMAC secret, session names, invitation contents, offline data, or inner bytes. Route-key rotation registers old+new for a bounded overlap, switches clients, then revokes old; challenge signatures, nonce/expiry/audience binding, replay cache, one-use notices, compression-off, buffers/rates, and no payload logs are contract-tested. The Mac remains inner TLS server: QR-pinned server-auth in pairing, mTLS afterward.
...
One shared classifier/Unix-socket broker enforces versioned policy. The host checks direct RPC bash and destructive bridge actions. Agent tool/user-bash interception uses an integrity-pinned minimal Pi 0.84 patch: after **all** ordinary extension handlers have mutated input, Pi calls a preload-registered immutable out-of-band client with the final arguments. `NODE_OPTIONS` loads it before Pi, and every nested in-process `AgentSession` invokes the same patched core even when created with `extensions:false`. Source locator, patch hash, and nested-session tests block Pi drift.
