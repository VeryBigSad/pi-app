...
**Distribution.** The signed APK is mirrored to releases of the public repository `VeryBigSad/pi-app-releases`, so downloads are anonymous. Publication is a manual `workflow_dispatch` GitHub workflow that builds/verifies the signed APK (protected Environment secrets when present, otherwise a pre-signed artifact verified fail-closed with `apksigner` against the pin), verifies anonymous download, generates `update-v1.json` deterministically, and commits it to `VeryBigSad/verybigsad.github.io`.
