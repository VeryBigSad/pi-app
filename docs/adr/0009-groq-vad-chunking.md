...
Capture 16 kHz mono signed PCM only while foregrounded. Android VAD keeps 300 ms pre-roll, starts on speech, prefers silence after 8 seconds, forces a cut at 12 seconds, and overlaps about 500 ms; the Mac accepts complete bounded chunks and never rechunks. One request is in flight, at most two queue, responses emit strictly in order, and stop flushes the final speech fragment. Seam matching compares Unicode/case/punctuation-normalized tokens while retaining the original output spans. Any chunk failure terminally aborts the active and queued stream work, emits exactly one error, and suppresses late partial/final results.

A durable Mac ledger reserves before every upload attempt, including retries. It counts request attempts, encoded audio seconds including overlap, and conservative billed seconds `max(encodedDuration, 10s)`. Defaults retain 10% headroom: 18 RPM, 1,800 RPD, 6,480 audio seconds/hour, and 25,920 audio seconds/day. The user may change them only to explicit current organization limits. Each immediate SQLite reservation transaction persists `effectiveNow = max(wallClockNow, previousEffectiveNow)`; rollback, restart, and concurrent connections cannot reopen sliding windows or UTC budgets, while a forward jump remains pinned conservatively. Any exhausted window stops capture before upload and shows its reset.
