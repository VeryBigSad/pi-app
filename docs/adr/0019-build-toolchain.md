...
Node is exactly `22.23.2` with npm `10.9.8`, one exact-version workspace lock, TypeScript `6.0.3`, and native-dependency packaging smoke. Go keeps the `go 1.26.0` language-version floor and pins the compiler toolchain to `go1.26.5` in the workspace, relay module, CI, and digest-pinned official multi-architecture build image (`golang:1.26.5@sha256:7caba5286b4c3613a337b709c573047d8ae62ee76106647313b61e72b99f20af`). This remediates GO-2026-5037 and GO-2026-4971 without advisory suppression. Terraform code supports the installed `1.5.7` floor but CI/applies use a separately pinned current 1.x binary and committed provider lock.
