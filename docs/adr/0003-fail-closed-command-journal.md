...
Dispatch is at most once but may execute zero times. Dormant RECEIVED can resume only with full current revalidation; indeterminate ARMED requires inspection and a new command. Terminal input is not replayed. Crash injection around every state/write boundary is a release gate.
