...
For local development with a debug-signed APK (for example on an emulator whose Play services is too old for real passkeys), the debug app computes its own `android:apk-key-hash:` origin from its signing certificate and runs passkey ceremonies through a debug-gated local authenticator that still produces fully valid WebAuthn responses. The Mac daemon only accepts such debug origins when `config.json` explicitly lists them in `debugAndroidOrigins` (exact `android:apk-key-hash:<base64url>` strings); with the key unset, only the release origin is accepted. Release builds ignore both hooks entirely. This is a local-development escape hatch, never a production configuration.
